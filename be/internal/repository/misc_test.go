...
func TestNewsListActive(t *testing.T) {
	m := newPool(t)
	...
	r := NewNewsRepo(m)
	ns, err := r.ListActive(context.Background())
	if err != nil {
		t.Fatalf("ListActive: %v", err)
	}
	if len(ns) != 1 || ns[0].Title != "Update" {
		t.Fatalf("ns = %+v", ns)
	}
}
...
func TestNewsCreateUpdateDelete(t *testing.T) {
	m := newPool(t)
	title := "Judul baru"
	m.ExpectQuery(`INSERT INTO news`).WithArgs("Judul", "isi").
		WillReturnRows(pgxmock.NewRows([]string{"id", "title", "content", "isActive", "createdAt", "updatedAt"}).
			AddRow(int32(1), "Judul", "isi", true, time.Now(), time.Now()))
	m.ExpectQuery(`UPDATE news SET`).WithArgs(int32(1), &title, (*string)(nil)).
		WillReturnRows(pgxmock.NewRows([]string{"id", "title", "content", "isActive", "createdAt", "updatedAt"}).
			AddRow(int32(1), "Judul baru", "isi", true, time.Now(), time.Now()))
	m.ExpectExec(`UPDATE news SET "deletedAt"`).WithArgs(int32(1)).
		WillReturnResult(pgxmock.NewResult("UPDATE", 1))
	r := NewNewsRepo(m)
	n, err := r.Create(context.Background(), "Judul", "isi")
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	n2, err := r.Update(context.Background(), 1, &title, nil)
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
	if n2.Title != "Judul baru" {
		t.Errorf("title = %q", n2.Title)
	}
